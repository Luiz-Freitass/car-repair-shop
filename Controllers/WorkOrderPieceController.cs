...
public class WorkOrderPieceController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> CreateWorkOrderPiece(CreateWorkOrderPieceDto dto)
    {
        var workOrderPiece = _mapper.Map<WorkOrderPiece>(dto);
        _appDbContext.WorkOrderPieces.Add(workOrderPiece);
    ...
    {
        var workOrderPieces = await _appDbContext.WorkOrderPieces
            .Skip(skip)
            .Take(take)
            .ToListAsync();
    ...
    {
        var workOrderPiece = await _appDbContext.WorkOrderPieces
    ...
    public async Task<IActionResult> DeleteWorkOrderPiece(int workOrderId, int pieceId)
    {
        ...
        _appDbContext.WorkOrderPieces.Remove(workOrderPiece);
        await _appDbContext.SaveChangesAsync();
