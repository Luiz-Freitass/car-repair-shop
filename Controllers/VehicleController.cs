...
public class VehicleController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateVehicle([FromBody]CreateVehicleDto dto)
    {
        ...
        var readVehicle = _mapper.Map<ReadVehicleDto>(vehicle);
        return CreatedAtAction (nameof(GetVehicleById), new {id = vehicle.Id }, readVehicle);
    }
    ...
    {
        var vehicle = _appDbContext.Vehicles
            .Include(v => v.Client)
    ...
    {
        var vehicle = await _appDbContext.Vehicles.FirstOrDefaultAsync(v => v.Id == id);
        if (vehicle == null) return NotFound();

        var clientExists = await _appDbContext.Vehicles.AnyAsync(c => c.Id == dto.ClientId);
