...
public class WorkOrderController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateWorkOrder(CreateWorkOrderDto dto)
    {
        ...
        return CreatedAtAction(nameof(GetWorkOrderById), new { id = workOrder.Id }, readWorkOrder);
    }

    [HttpGet]
    public async Task<IActionResult> GetAllWorkOrders([FromQuery] int skip = 0, [FromQuery]int take = 20)
    {
        var workOrders = await _appDbContext.WorkOrders
            .Skip(skip)
            .Take(take)
            .ToListAsync();
        return Ok(_mapper.Map<List<ReadWorkOrderDto>>(workOrders));
    }
